...
void Socket::GetPeerName(sockaddr_storage & addr) const
{
	...
	Lock();
	bool suc = getpeername(m_socket, (sockaddr*)&addr, &addSize) == -1;
	Unlock();
	if (suc)
	{
...
Socket::Address Socket::GetAddress() const
{
	...
	catch (const cg::ex::Exception& err)
	{
...
bool Socket::ReadReady(cg::Timeout timeoutp) const
{
...
bool Socket::WriteReady(cg::Timeout timeoutp) const
{
	auto timeout = (ptrdiff_t)timeoutp;
	timeval to = timeval();
	int code;
