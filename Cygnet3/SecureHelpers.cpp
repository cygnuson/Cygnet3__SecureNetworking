	...
	const CryptoPP::SecByteBlock& iv)
{
	...
	if (key.size() == 0)
		throw ExEncryption(ExEncryption::Code::BadKey);
	if (iv.size() == 0)
		throw ExEncryption(ExEncryption::Code::BadIv);
	if (sSize == 0)
		throw ExEncryption(ExEncryption::Code::NoData);
	...
	bool clobber)
{
	if (!clobber)
	{
		...
		if (ifs.good())
			throw ExEncryption(ExEncryption::AlreadyExists);
	}
...
std::string ExEncryption::What() const
{
	return ToString();
}

std::string ExEncryption::ToString() const
{
	switch (m_code)
	{
	case cg::ExEncryption::BadIv:
		return "The IV is invalid.";
	case cg::ExEncryption::BadKey:
		return "The Key is invalid.";
	case cg::ExEncryption::NoData:
		return "There is no data to encrypt.";
	case cg::ExEncryption::AlreadyExists:
